...
await mkdir('artifacts',{recursive:true});const folder=await mkdtemp(resolve('artifacts/return-astra-')),base=join(folder,'base');
...
async function scenario(id,fn){phase=id;calls=[];const dir=join(folder,id);await cp(base,dir,{recursive:true,errorOnExist:true,force:false});at+=60000;
...
try{
 ...
 s.receiveSpeech({id:randomUUID(),sessionId:s.sessionId,text:'합성 암호는 해달우체통이에요. 들으신 분?',source:'microphone',capture});await s.react({});
 result('delayed-delivery',!calls[0].args.settings.personas.some(p=>p.id==='new')&&!s.journal.data.entries.find(e=>e.text.includes('해달우체통')).witnesses.includes('new'),{messages:calls[0].observation.messages});deliver(s);await close();await save();
 ...
 await scenario('unheard-memory',async s=>{await s.react({speech:'오늘처음옴님, 자리 비웠을 때 제가 말한 암호도 들었어요?'});const text=targetText(calls[0],'new');result(phase,!!text&&!text.includes('해달우체통')&&/못|몰라|모르|안 들|없|비웠/.test(text),{text});});
 ...
 report.passed=report.results.length===7&&report.results.every(r=>r.passed);
