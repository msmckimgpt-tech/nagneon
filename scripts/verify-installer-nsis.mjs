...
try{
  ...
  for(const name of ['install','refresh']){
    ...
    const ext=await external(name);assert.equal(ext.registries.Registry64.InstallLocation,target);assert.equal(ext.shortcut.target,join(target,'Nagneon Launcher.exe'));
  ...
  const child=spawn(join(target,'Nagneon Launcher.exe'),[],{windowsHide:true,stdio:'ignore',env:{...process.env,BACKSEAT_FIXTURE_CONTROL:control}});
