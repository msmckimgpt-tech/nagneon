...
const report={base,passed:false,nativeWindows:true,systemAudio:false,microphone:false,liveModel:false,checks:[],connections:[]};let service,main;const fixtures=[];
...
app.whenReady().then(async()=>{try{
 ...
 service=await startServer({port:0,dataDir:join(base,'data'),localSpeech:false,provider:{status:()=>({configured:true}),react:async args=>{if(args.image)seenFrames++;return {observation:{game:'Owned native fixture',scene:'Local fixture validation',confidence:1,excitement:0,messages:[]}};}}});
 ...
 const js=code=>main.webContents.executeJavaScript(code,true),pause=ms=>new Promise(r=>setTimeout(r,ms));
 ...
 for(const [index,fixture] of fixtures.entries()){
  await js(`document.querySelector('[title="화면 선택"]').click();`);
  ...
  if(index===0)await until(()=>Promise.resolve(seenFrames>0));
 }
 report.checks.push('actual selected native frames reach the live observation path and match fixture pixels');
 report.checks.push('second native source replaces the first only when ready and stops the old track');
 fixtures[1].close();await until(()=>js(`!document.body.textContent.includes('연결 해제')&&previousCapture.getTracks().every(t=>t.readyState==='ended')`));
