...
Please use [GitHub private vulnerability reporting](https://github.com/msmckimgpt-tech/streamer/security/advisories/new) to report suspected vulnerabilities. Include the affected commit or version, reproduction steps using synthetic data, and the impact.
...
BACKSEAT is a local desktop application. Its server is intended to listen only on `127.0.0.1`, with a per-process authentication capability. Do not expose it through port forwarding, a public reverse proxy, or a network-wide listen address.
