...
실제 변경의 최종 답변은 짧은 요약 뒤 하나의 Markdown 코드 블록으로 인계 전문을 제공한다.
제품/M1 구분, 목표·범위·금지, Personal Commander workspace, M1 revision/read set, branch/worktree·기반/최종 SHA·commit,
변경 파일/영향, 검증 명령/exit code/증거, 실패/미실행, B의 선행 조건·통합/적용/복구 기준을 실제 값으로 적는다.
