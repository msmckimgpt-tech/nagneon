...
현재 기능별 증거와 남은 실제 검증은 [수용 현황](BENCHMARK-ACCEPTANCE.md)에서 확인할 수 있다.

최신 설치 수용: `artifacts/benchmark-full-installer-longpath/Nagneon-Setup.exe` 컴파일 및 전체 2,452개 파일의 TEST 설치·설치된 앱 실행/정상 종료·제거·사용자 파일 보존이 통과했다. 원본은 `artifacts/installer-full-test-2026-09-14T14-21-45-763Z`에 있다. payload 기능 소스는 디버그 포함 `dd51e97`이며 사용자 앱을 교체하지 않았다. 상세 크기·해시·단계별 증거와 배포 범위는 [설치 기록](INSTALLER.md)에 정리했다. 외부 플랫폼의 실제 승인/채팅, 사용자 참여 게임/마이크, 로컬 모델 지원 수준 결정은 남아 있다.
