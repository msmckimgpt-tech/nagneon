...
| 번호 | 지적 | 상태 |
|---|---|---|
| 1-1 | 리브랜딩이 절반만 됐고, 그게 화면에 보인다. | 수정: 주요 강조색 통일 및 style.css 색상 리터럴을 의미별 토큰으로 정리. 실제 렌더러에서 171개 변수와 주요 강조색 검증(BRAND-COLORS.md) |
...
| 1-7 | 오버레이가 방송 송출용인지 개인 모니터용인지 불명확하다. | 수정: 개인/공개 선택과 AI·가상 포인트 표시. OBS WGC 실제 캡처 확인. 개인용 전환 시 마지막 공개 프레임 잔류 안내(OBS-OVERLAY-ACCEPTANCE.md); 패키지·송출 검증은 별도 |
...
| 3-1 | 컨텐츠 총량을 세어보면 초라하다. | 부분 개선: 사용자 지정 공개 커뮤니티의 문화 참고자료를 기존 AI 연결로 분석·활용(CULTURE-LEARNING.md). 콘텐츠 품질·장르 구성은 별도 미완료 |
| 3-2 | 컨텐츠를 추가하는 방법이 "코드 수정"뿐이다. | 부분 개선: 설정에서 문화 참고 도메인을 추가할 수 있음. 앰비언트·유입 경로·경제 규칙의 범용 편집 기능은 미구현 |
...
| 3-6 | 게임 프로필 4개 중 3개가 한물간 선정이다. | 장르 관찰 기본값 6종 추가. 자동 인식·기존 게임 유지, 기존 사용자는 설정에서 누락 항목만 추가·저장(GAME-GENRE-PRESETS.md). 실제 모델의 장르별 반응 품질은 별도 평가 |
| 4-1 | 제품 이름이 두 개다. | 1차: 사용자 JSON 파일명을 nagneon으로 변경. 내부 호환 식별자는 보존 |
| 4-2 | 팔 수 있는 상태가 아닌데 팔 준비 문서만 있다. | 사용자 결정: 무료 공개·MIT 유지. 0.1.4 실제 배포·업데이트·복귀 및 사용자 적용 검증. 유료 판매는 현재 범위 제외 |
| 4-3 | 사용자가 비용을 두 번 낸다. | 정리: 앱 무료·가상 포인트 현금 구매 없음, AI 제공처 계정·구독 조건은 별도임을 README에 명시 |
...
| 5-2 | 포인트가 안 모인다. | 개선: 새 프로필 200P·기존 잔액 유지. 조용한 의미 있는 교류도 자발적 후원 대상에 포함, 높은 흥분도·impact의 지급 차단 제거(QUIET-CONVERSATION-DONATIONS.md). 실제 장기 획득 속도는 미측정 |
| 5-3 | 20초 늦는 관객은 게임 반응으로 쓸 수 없다. | UI 문구: 즉시 반응으로 오해할 수 있는 '실시간 채팅'을 '관객 채팅'으로 변경. 모델 응답 지연 개선은 별도 미완료 |
...
| 5-7 | 마이크가 GPU를 요구한다. | 사실 정정·완화: CPU 선택과 폴백 지원. 실제 medium CPU 실행 검증, 게임 자원 경쟁/CPU 지연 선택 안내 추가. 동시 게임 FPS는 미측정(CPU-SPEECH.md) |
...
| 6-2 | 13,004자 프롬프트가 매 요청 전송된다. | 부분 개선: 완전히 같은 공개 문맥을 관객 ID별 목격 범위를 유지하며 묶어 전달. 실제 구독 모델의 합성 사례 4회 비교에서 입력 합계 40.4% 감소(TOKEN-PRESSURE.md). 개인 기억·신규 관객 구분 유지. 응답 시간 개선은 관측되지 않았고 구독 한도·금전 비용 효과는 미측정 |
...
| 6-6 | 이미지가 매 프레임 base64로 왕복한다. | 부분 개선: JPEG 비동기·단일 변환과 늦은 결과 제외 유지. 네이티브 변환 지원 렌더러는 바이너리 HTTP 전송으로 합성 본문 약 25% 감소(FRAME-TRANSPORT.md). 서버 이후 data URL 호환은 유지하며 왕복·모델 지연의 일관된 개선은 미입증 |
| 6-7 | 실패 백오프가 관대하다. | 수정: 간헐적 성공에도 실패 단계 유지, 연속 3회 정상 응답 후 초기화. 6→12→24→48→60초 상한 유지, 동일 화면 생략은 회복으로 세지 않음. 실패 시도는 사용량에 남지만 세션 횟수 제한으로 방송을 중단하지 않음 |
| 8-1 | 내 채팅이 내 동의 없이 OpenAI로 간다. | 수정: 전송 안내·확인, 기본 작성자 익명화, 명시적 닉네임 선택. 실연결 검증 남음 |
| 8-2 | 스트리머가 나를 무시하고 봇에게 답한다. | 대조·개선 예정 |
| 8-3 | 지연이 나에게는 이상하게 보인다. | 부분 개선: 공개 오버레이에 이전 장면·대화에 대한 늦은 반응 가능성 표시. README에 AI 응답이 플랫폼 채팅에 자동 게시되지 않는 점과 응답 대상 설명 안내. 모델 지연 자체와 실제 시청자의 이해도는 별도 미검증 |
| 8-4 | 스트리머가 가짜 후원에 감사 인사를 한다. | 수정: 공개용 가상 포인트 후원 표시. 합성 25P 알림의 실제 OBS WGC 캡처에서 실제 금전 후원이 아니라는 문구 확인. 토스트가 고정 공개 안내를 가리던 배치도 수정(OBS-OVERLAY-ACCEPTANCE.md). 모델 후원 생성·플랫폼 송출 시험은 아님 |
| 8-5 | 인원수가 안 맞는다. | 수정: 공개용 AI 관객 명시. OBS WGC 캡처에서 플랫폼 실제 시청자·후원이 아니라는 고정 안내 확인 |
| 8-6 | 들키는 방식이 가장 나쁘다. | 수정: 공개 모드 AI/가상 포인트 고정 안내의 OBS WGC 캡처 확인. 다른 방식·실제 송출은 미검증 |
...
설정 설명·색상 토큰·비동기 화면 변환·관객 채팅 명칭·포인트 상태 복사·방송 세션 타입·음성 입력 상태 발행·README 안내 GIF를 함께 검증했다. 새 문화 참고자료 기능의 설정과 관객별 사용 기록 처리를 보존한다. 포인트 정책이나 관객 수 상한을 추가하지 않는다.

- 최종 `npm run check`: format, 762/762 테스트, TypeScript/Vite 빌드 통과. 의존성 audit 0건.
- 실제 비표시 Electron에서 설정 설명·좁은 화면 및 171개 색상 변수 검증 통과. 합성 캔버스 검사는 이미지 바이트 동일성과 이벤트 처리 대기 감소를 확인했으며 전체 인코딩 완료 시간 개선으로 해석하지 않는다.
- 실제 인증된 로컬 HTTP/SSE와 합성 발언으로 완성된 음성 상태 한 번 발행을 확인했다. 실계정·물리 장치 검사는 아니다.
- 중간 전체 검사에서 기존 튜토리얼 재시작 테스트의 `fetch failed` 1건이 발생했다. 해당 8개 검사와 전체 재검사·최종 검사는 통과했다. 간헐적 원인은 미확정이며 실패 원본을 보존한다.
- 원본: `review-followups-integration-20260923/artifacts/`의 `check-final.log`, `check-state.log`(실패), `tutorial-focused.log`, `check-state-recheck.log`, `settings-palette-ui.log`, `palette-ui-recheck.log`, `canvas-native.log`, `speech-http.log`, `audit-final.log`. 최초 색상 검사는 검증용 목록 파일 누락으로 중단됐고 목록을 복원한 재검사에서 통과했다.
- README의 GIF는 합성 관객·대화를 사용한 편집 안내다. 실제 AI 응답 지연·OBS 송출 수용 검증이나 신규 패키지 배포 완료를 의미하지 않는다.
