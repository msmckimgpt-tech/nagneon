...
복구는 `desktop/profile-recovery.cjs`에서 앱 서버 시작 전에 수행한다. Nagneon.exe 직접 실행과 고정 실행기 모두 같은 경로를 사용한다. 프로필이 없고 같은 상대 경로의 패키지 LocalCache 기록이 하나면 원본 보존·전체 파일 해시 검증 후 자동 복구하고 방송실을 연다. 후보가 여러 개이거나 등록 설정이 손상됐거나 저장 장치가 없으면 앱 안에서 기존 기록 폴더 선택/다시 찾기를 제공한다. 선택한 프로필로 연결하고 기존 storage.json은 별도 보존한다. 잘못 선택해도 복구 화면으로 돌아와 재선택할 수 있다. 사용자가 종료를 선택했을 때만 복구 흐름을 취소한다.

진짜 첫 실행에는 기본 프로필을 사용한다. 이미 존재하는 data를 덮어쓰거나 다른 기록으로 자동 교체하지 않는다. 검증용 명시 프로필은 전역 설정을 바꾸지 않는다. 앱 로그의 `profile-recovery.jsonl`에는 복구 완료 시각과 파일 수만 기록한다.

PowerShell의 별도 복구 구현은 제거했으며, 0.1.6 이상 실행기는 저장 프로필 검사를 앱에 맡긴다. 이전 `v0.1.5-launcher.1` 유지보수 릴리즈·첨부 파일·태그는 사용자 요청으로 철회했다. 아래 2026-09-16 실행기 복구 기록은 역사적 검증이며 현재 배포/복구 방법이 아니다.
...
- 선택한 위치는 `%APPDATA%\Nagneon\storage.json`에 버전과 독립적으로 저장한다. 저장 장치가 없거나 설정이 손상되면 앱 내부에서 기존 기록을 다시 연결한다.
