...
사용자에게는 안전한 종료도 실행 실패이므로, 일반 실행에서는 누락된 기본 프로필과 같은 상대 경로의 패키지 `LocalCache/Roaming` 기록을 찾는다. `world.json`이 있는 후보가 하나면 원본을 남긴 채 임시 폴더에 복사하고 파일별 SHA-256과 목록을 대조한 뒤 `data`로 게시하여 앱 실행을 계속한다. 이미 존재하는 `data`는 덮어쓰지 않는다. 후보가 없거나 여러 개면 기존 프로필/기록 폴더를 선택하는 창으로 연결하며 임의로 과거 백업을 선택하지 않는다. 사용자가 취소하거나 기록 자체가 읽히지 않는 경우만 복구 안내를 표시한다. `-Inspect`는 읽기 전용이며 자동 복구를 실행하지 않는다.

배포된 0.1.5 실행 파일에도 설치 루트의 실행기·호환성 도우미를 교체하여 적용할 수 있다. 이는 앱 바이너리 변경과 별도인 설치 도구 유지보수다. 등록/JSON 손상/잘못된 경로는 PowerShell 스택 대신 복구 안내로 처리한다. 존재하지 않는 프로필의 자동 초기화와 오류 무시는 하지 않는다.

검증: 688개 테스트와 빌드 통과(`check-recovery-final.log`). 실제 탐색기에서 격리 복구 시험 실행기를 더블클릭하여 가상 AppData에만 있던 339개 파일을 자동 복구했다. 네이티브 프로세스의 `artifacts/native-auto-recovery/native-result.json`은 `recovered=true`와 원본 일치 SHA-256을 기록했고, 설치된 0.1.5 앱의 격리 프로필 화면에서 관객 6명/103포인트를 직접 확인했다. 사용자 기본 프로필을 삭제하거나 손상시켜 재현하지 않았다. 기본 프로필과 시험 프로필의 원본은 보존한다.
