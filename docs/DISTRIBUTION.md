...
현재 공개 버전은 [0.1.6](RELEASE-0.1.6.md)입니다. 기본 앱과 선택형 런타임의 크기·빌드 방법은 [배포 용량](DISTRIBUTION-SIZE.md)을 따릅니다. `npm run package:windows`는 경량 앱을 기본으로 만들며 로컬 음성 모델 준비가 필요 없습니다. 전체 동봉이 필요할 때만 `-- --bundled --speech=<검증된 런타임 폴더>`를 지정합니다.
