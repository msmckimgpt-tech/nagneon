...
type Config={enabled:boolean;mode:'append'|'replace';prompt:string};
...
export function DebugPanel({locked,onSettingsSaved}:{locked:boolean;onSettingsSaved:()=>void}){
  const [view,setView]=useState<Snapshot|null>(null),[config,setConfig]=useState<Config>({enabled:false,mode:'append',prompt:''}),[raw,setRaw]=useState(''),[pending,setPending]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState('');
  ...
  return <div className="debug-panel">
    ...
    <fieldset disabled={locked||pending||!view}>
      ...
      {view?.config.enabled&&<>
        ...
        <div className="connection-actions"><button className="secondary" onClick={()=>void saveConfig(config)}>프롬프트 적용</button><button className="secondary" onClick={()=>setConfig({...config,mode:'replace',prompt:view.basePrompt||''})}>기본 프롬프트를 편집창에 불러오기</button><button className="text-button" onClick={()=>void saveConfig({enabled:true,mode:'append',prompt:''})}>프롬프트 기본값 복원</button></div>
