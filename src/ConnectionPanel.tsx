...
export function ConnectionPanel({state}:{state:State}){
  ...
  return <section className="account-panel" aria-label="계정과 모델 연결">
    ...
    <p className="account-note">{state.provider.kind==='ollama'?'이 PC의 Ollama 모델을 사용합니다. 한국어 품질과 속도는 모델과 PC 성능에 따라 달라요. 로컬 모드에서는 웹 검색을 지원하지 않아요.':'ChatGPT 구독의 Codex 사용량을 이용합니다. 계정 연결만으로 이 모델의 접근 권한이나 남은 사용량이 보장되지는 않습니다.'}</p>
    <div className="connection-actions">
      ...
      {state.provider.configured&&<button className="secondary" disabled={disabled} onClick={()=>void perform(()=>api('connection/probe'))}><Radio size={14}/> {state.provider.kind==='ollama'?'로컬 모델 응답 확인 · 1회':'Astra 응답 확인 · 1회 사용'}</button>}
    </div>
    ...
    {checking&&<div className="probe-result" role="status"><LoaderCircle size={17} className="spin"/><div><b>Astra의 첫 인사를 기다리는 중</b><p>화면·음성·기존 대화 없이 짧은 시험 요청 하나를 보냅니다.</p></div><button className="text-button" onClick={()=>void api('connection/probe/cancel').catch(e=>setError(e.message))}>응답 확인 취소</button></div>}
