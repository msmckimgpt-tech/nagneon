...
export function ConnectionPanel({state}:{state:State}){
  ...
  return <section className="account-panel" aria-label="계정과 모델 연결">
    <div className="account-summary"><span className={'connection-orb '+(state.provider.configured?'ready':'')}><Radio size={20}/></span><div><b>{state.provider.authMessage||(state.provider.configured?'AI 연결 준비됨':'AI 연결이 필요합니다.')}</b><p>{state.provider.model}{state.provider.kind!=='antigravity'&&` · 추론 ${state.provider.effort}`} </p></div>{state.provider.configured&&<Check size={20}/>}</div>
    <p className="account-note">{state.providerChoice?.routing?'역할별로 지정한 연결을 같은 대화와 기록에 사용합니다. 각 연결의 구독·API 사용량이 적용됩니다.':state.provider.kind==='ollama'?'이 PC의 Ollama 모델을 사용합니다. 한국어 품질과 속도는 모델과 PC 성능에 따라 달라요. 로컬 모드에서는 웹 검색을 지원하지 않아요.':state.provider.kind==='antigravity'?'Antigravity에 연결한 Google 계정의 Gemini 사용량을 이용합니다. API 키는 필요하지 않습니다.':state.provider.kind==='openai'?'OpenAI API 사용량으로 별도 청구됩니다.':'ChatGPT 구독의 Codex 사용량을 이용합니다. 계정 연결만으로 이 모델의 접근 권한이나 남은 사용량이 보장되지는 않습니다.'}</p>
    ...
    {state.provider.kind==='antigravity'&&!state.provider.configured&&<p className="field-note">Antigravity CLI에서 Google 계정으로 로그인한 뒤 ‘연결 상태 새로고침’을 누르세요.</p>}
