...
export function ConnectionPanel({state}:{state:State}){
  ...
  return <section className="account-panel" aria-label="계정과 모델 연결">
    ...
    <p className="account-note">{state.provider.kind?.endsWith('-cli')?'설치된 공식 CLI의 로그인 계정으로 요청합니다. 응답 확인에서 로그인·모델 권한·사용량을 확인하세요.':state.provider.kind==='ollama'?'이 PC의 Ollama 모델을 사용합니다. 한국어 품질과 속도는 모델과 PC 성능에 따라 달라요. 로컬 모드에서는 웹 검색을 지원하지 않아요.':['openai','claude','gemini'].includes(state.provider.kind||'')?'선택한 제공처의 API 사용량으로 별도 청구됩니다.':'ChatGPT 구독의 Codex 사용량을 이용합니다. 계정 연결만으로 이 모델의 접근 권한이나 남은 사용량이 보장되지는 않습니다.'}</p>
    {state.debug?.previewEnabled&&<ProviderPicker state={state} disabled={disabled}/>}
    <div className="probe-result" aria-label="세션 사용량"><div><b>세션 모델 요청 {state.calls.toLocaleString('ko-KR')} / {state.settings.maxCalls.toLocaleString('ko-KR')}회</b><p>제공처가 보고한 누적 토큰 {state.tokens.toLocaleString('ko-KR')}개</p><small>실패한 요청도 횟수에 포함될 수 있어요. 연결 시험과 계정 전체 사용량은 별도이며, 토큰 수는 요금이나 남은 구독량을 뜻하지 않습니다.</small></div></div>
    ...
    {state.provider.kind==='codex'&&!state.provider.configured&&!window.backseat&&<p className="field-note">계정 로그인은 Windows 데스크톱 앱에서 진행하세요.</p>}
