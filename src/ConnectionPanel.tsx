...
export function ConnectionPanel({state}:{state:State}){
  ...
  return <section className="account-panel" aria-label="계정과 모델 연결">
    ...
    <p className="account-note">ChatGPT 구독의 Codex 사용량을 이용합니다. 계정 연결만으로 이 모델의 접근 권한이나 남은 사용량이 보장되지는 않습니다.</p>
    <div className="connection-actions">
      ...
      {state.provider.configured&&<button className="secondary" disabled={disabled} onClick={()=>void perform(()=>api('connection/probe'))}><Radio size={14}/> Astra 응답 확인 · 1회 사용</button>}
    </div>
    {!state.provider.configured&&!window.backseat&&<p className="field-note">계정 로그인은 Windows 데스크톱 앱에서 진행하세요.</p>}
