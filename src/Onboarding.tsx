...
export function Onboarding({state,onDone}:{state:State;onDone:()=>void}){
  ...
  return <div className="welcome-shell">
    <header className="welcome-header"><span className="brand"><Radio size={26}/> backseat<span className="brand-dot">.</span></span><button className="text-button" disabled={busy} onClick={()=>void finish()}>나중에 설정하기</button></header>
    <main className="welcome-main"><div className="welcome-intro"><span className="eyebrow">YOUR FIRST BROADCAST</span><h1 tabIndex={-1} ref={heading}>{['당신이 켜면, 이야기가 시작돼요.','어떤 채팅창을 꿈꾸셨나요?','관객이 인사할 준비를 할게요.'][step]}</h1><p>{['게임 한 판, 늦은 밤 수다, 기다려온 기념 방송까지.','관객마다 성격은 다르게, 함께하는 분위기는 당신답게.','지금 연결하거나, 사용량 없는 리허설로 먼저 둘러보세요.'][step]}</p></div>
    ...
    <div className="welcome-columns"><section className="welcome-card">
      ...
      {step===2&&<><ConnectionPanel state={state}/><div className="first-stream-note"><b>입장한 뒤, 직접 방송을 시작하세요.</b><p>마이크와 화면은 각각 연결 버튼을 눌렀을 때만 켜집니다. 마이크 소리는 이 PC에서 글로 바뀌며, 켠 화면의 이미지와 대화가 AI에 전달됩니다.</p><p>리허설은 준비된 예시 채팅으로 방송실을 체험합니다. 실제 화면 인식과 자유 대화는 AI 방송에서 이용하세요.</p></div></>}
    ...
    </section><aside className="welcome-preview"><div className="welcome-preview-title"><span className="dot green"/><b>{title||'나의 첫 방송'}</b><small>분위기 예시</small></div><div className="welcome-sample"><span style={{color:'#a89bff'}}>모모</span><p>{name||'방장'} 왔다! 오늘은 뭐 하면서 놀아요?</p></div><div className="welcome-sample"><span style={{color:'#f18fac'}}>팝콘도둑</span><p>{vibe==='stadium'?'오늘 명장면 하나 나올 느낌인데 ㅋㅋ':vibe==='lively'?'첫 채팅 선점 성공 ㅋㅋ 팝콘 준비됐다':'일단 의자부터 당겨야지 🍿'}</p></div><div className="welcome-sample"><span style={{color:'#99d9af'}}>루나 · 매니저</span><p>{advice==='on-request'?'힌트는 방장이 부탁할 때 같이 생각해봐요.':'다들 편하게 즐겨요. 스포일러는 조심!'}</p></div><div className="welcome-promise"><Sparkles size={24}/><h2>관객과 함께 쌓는 세계</h2><p>기억에 남는 순간은 핫클립으로,<br/>방송 밖 이야기는 커뮤니티로.<br/>팬 페스티벌과 기념 방송도 기다리고 있어요.</p><small>모든 관객과 후원은 가상입니다.<br/>실제 생방송 송출이나 현금 결제는 일어나지 않아요.</small></div></aside></div>
