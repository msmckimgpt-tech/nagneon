...
export function Onboarding({state,onDone}:{state:State;onDone:()=>void}){
  ...
  async function finish(mode?:Settings['mode']){
    ...
    try{await api('onboarding',mode?{streamer:name,title,category,crowdStyle:vibe,streamerStyle:style,adviceMode:advice,mode}:{skip:true});await api('tutorial',{action:mode?'begin':'skip'});onDone();}
  ...
  return <div className="welcome-shell">
    ...
    <div className="welcome-columns"><section className="welcome-card">
      ...
      {step===2&&<><ConnectionPanel state={state}/><div className="first-stream-note"><b>입장한 뒤, 직접 방송을 시작하세요.</b><p>튜토리얼은 리허설로 진행하며 마이크와 화면을 켜지 않습니다. 실제 AI 방송을 시작하면 마이크가 자동 연결되고, 화면은 직접 선택할 때 공유됩니다. 마이크 소리는 이 PC에서 글로 바뀌며, 켠 화면의 이미지와 대화가 AI에 전달됩니다.</p><p>리허설에서 채팅과 화면 배치를 살펴보고 첫 방송을 준비하세요.</p></div></>}
      {error&&<p role="alert" className="connection-problem">{error}</p>}
      <div className="welcome-actions">{step>0&&<button className="secondary" disabled={busy} onClick={()=>setStep(step-1)}><ArrowLeft size={15}/> 이전</button>}{step<2?<button className="primary" disabled={!ready||busy} onClick={()=>setStep(step+1)}>다음 <ArrowRight size={15}/></button>:<><button className="primary" disabled={busy||!ready} onClick={()=>void finish('live')}>직접 조작하며 배우기 <ArrowRight size={15}/></button></>}</div>
    </section><aside className="welcome-preview"><div className="welcome-preview-title"><span className="dot green"/><b>{title||'나의 첫 방송'}</b><small>처음 만날 방송실 안내</small></div><div className="welcome-sample"><span>방송 도우미</span><p>{name||'방장'}님, 방송실을 준비했어요. 리허설로 화면 배치와 입력을 먼저 살펴보세요.</p></div><div className="welcome-sample"><span>첫 관객과의 만남</span><p>AI 계정을 연결하고 관객을 초대하면 새로운 관객이 만들어져요. 이름과 성격은 관객마다 달라요.</p></div><div className="welcome-sample"><span>함께할 방송</span><p>{advice==='on-request'?'훈수는 요청할 때만, 이야기는 자유롭게 이어가세요.':'함께 볼 화면과 오늘의 이야기를 직접 골라보세요.'}</p></div><div className="welcome-promise"><Sparkles size={24}/><h2>처음엔 나그네, 어느새 단골</h2><p>기억에 남는 순간은 핫클립으로,<br/>방송 밖 이야기는 커뮤니티로.<br/>함께 나눈 대화가 우리만의 이야기가 됩니다.</p><small>처음엔 나그네,<br/>어느새 우리 단골.</small></div></aside></div>
