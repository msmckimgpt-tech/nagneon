...
export function Onboarding({state,onDone}:{state:State;onDone:()=>void}){
  ...
  return <div className="welcome-shell">
    ...
    <main className="welcome-main"><div className="welcome-intro"><span className="eyebrow">WELCOME TO NAGNEON</span><h1 tabIndex={-1} ref={heading}>{['방송을 켜면, 이야기가 찾아옵니다.','어떤 채팅창을 꿈꾸셨나요?','관객이 인사할 준비를 할게요.'][step]}</h1><p>{['나그네처럼 들러, 단골처럼 머무는 AI 관객들을 만나세요.','관객마다 성격은 다르게, 함께하는 분위기는 당신답게.','지금 연결하거나, 사용량 없는 리허설로 먼저 둘러보세요.'][step]}</p></div>
