...
import {VoiceBoundary,SpeechQueue,SpeechMailbox} from './speech-flow';
...
export function useMedia(state:State|null,onError:(s:string)=>void){
  ...
  const stateRef=useRef(state);stateRef.current=state;const pendingSpeech=useRef(new SpeechMailbox());const epoch=useRef(0),wake=useRef<()=>void>(()=>{});
  ...
  async function startMic(){
    ...
    try{
      ...
      const queue=new SpeechQueue<Blob,{text:string;cues?:{delivery?:string}}>({
        ...
        onResult:result=>{if(generation===epoch.current&&recording.current&&result.text){setTranscript(result.text);setDelivery(result.cues?.delivery||'');say(result.text);}},
      ...
      function segment(){
        ...
        rec.start();const timer=setTimeout(()=>{if(rec.state==='recording')rec.stop();},12000);
      }
      segment();
  ...
  useEffect(()=>{
    ...
    const tick=async()=>{
      const s=stateRef.current;if(disposed||inFlight||!s?.running||s.sessionId!==session||s.busy||s.calls>=s.settings.maxCalls)return;
      inFlight=true;const batch=pendingSpeech.current.batch();
      try{const result=await api<{skipped?:string}>('react',{image:s.settings.mode==='live'?frame():undefined,speech:batch.text});if(!result.skipped)pendingSpeech.current.acknowledge(batch.ids);}
