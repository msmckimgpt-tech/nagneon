...
import {api} from './api';
type Report={limit:number;summary:{attempts:number;retained:number;modelSilent:number;generated:number;delivered:number;pending:number;rejected:Record<string,number>;outcomes:Record<string,number>;providerP50Ms:number|null;providerP95Ms:number|null;decisionWaitP50Ms:number|null;decisionWaitP95Ms:number|null;modelP50Ms:number|null;modelP95Ms:number|null;firstChatSamples:number;firstChatWaitP50Ms:number|null;firstChatWaitP95Ms:number|null}};
...
export function ReactionDiagnostics(){
  ...
  return <details className="reaction-diagnostics" onToggle={e=>{if(e.currentTarget.open&&!report&&!loading)void refresh();}}><summary>채팅이 뜸하거나 늦을 때 · 응답 진단</summary>
    ...
    {report&&<><p>모델 호출 {report.summary.attempts}회 · 아래 집계는 최근 {report.summary.retained}회 기준</p><div className="tags"><span>모델이 채팅하지 않음 {report.summary.modelSilent}회</span><span>생성 {report.summary.generated}개</span><span>표시 {report.summary.delivered}개</span><span>대기 {report.summary.pending}개</span></div>
      <p>응답 준비 시간 · 중앙값 {seconds(report.summary.modelP50Ms)} / 95백분위 {seconds(report.summary.modelP95Ms)}</p>
      <p>주 모델 생성 · 중앙값 {seconds(report.summary.providerP50Ms)} / 95백분위 {seconds(report.summary.providerP95Ms)} · JEV 추가 대기 {seconds(report.summary.decisionWaitP50Ms)} / {seconds(report.summary.decisionWaitP95Ms)}</p>
      ...
      <p className="muted">응답 준비에는 주 모델 생성과 기다린 JEV 판단이 포함됩니다. 화면 반응의 JEV 판단은 별도로 진행해 추가 대기 없이 표시합니다. 마이크 전사·화면 연결·화면에 그리는 시간은 제외합니다. 채팅하지 않은 응답은 오류를 뜻하지 않습니다.</p>
