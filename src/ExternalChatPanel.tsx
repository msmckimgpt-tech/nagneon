...
export function ExternalChatPanel({state,onError}:{state:State;onError:(text:string)=>void}){
  ...
  async function connect(){const id=++ticket.current,controller=new AbortController();request.current=controller;setBusy(true);setAuthUrl('');
    try{const response=await fetch('/api/external/'+(platform==='chzzk'?'chzzk/start':'youtube/connect'),{method:'POST',headers:{'Content-Type':'application/json','X-Backseat-Client':'studio'},body:JSON.stringify(platform==='chzzk'?{clientId,clientSecret:secret}:{video,apiKey:key}),signal:controller.signal});const result=await response.json();if(!response.ok)throw Error(result.error);if(id===ticket.current&&result.authorizationUrl)setAuthUrl(result.authorizationUrl);}
  ...
  async function disconnect(){const id=++ticket.current;request.current?.abort();setBusy(true);setKey('');setSecret('');setAuthUrl('');try{await api('external/disconnect');}catch(e){if(id===ticket.current)onError(e instanceof Error?e.message:'외부 채팅 해제 실패');}finally{if(id===ticket.current)setBusy(false);}}
  ...
  return <details className="panel external-chat-panel"><summary>실제 방송 채팅 · 선택 사항</summary>
    ...
    {!connected&&!pending&&<><label>연결할 플랫폼<select aria-label="외부 채팅 플랫폼" value={platform} disabled={busy} onChange={e=>setPlatform(e.target.value)}><option value="youtube">YouTube</option><option value="chzzk">치지직</option></select></label>
      {platform==='youtube'?<><label>YouTube 방송 URL<input aria-label="YouTube 방송 URL" value={video} maxLength={2048} disabled={busy} onChange={e=>setVideo(e.target.value)}/></label><label>API 키<input aria-label="YouTube API 키" type="password" autoComplete="off" maxLength={256} value={key} disabled={busy} onChange={e=>setKey(e.target.value)}/></label><p className="muted">Google Cloud에서 YouTube Data API v3를 활성화하고 API 키를 만드세요. 키는 저장하지 않아요.</p></>:<><label>Client ID<input aria-label="치지직 Client ID" value={clientId} maxLength={512} disabled={busy} onChange={e=>setClientId(e.target.value)}/></label><label>Client Secret<input aria-label="치지직 Client Secret" type="password" autoComplete="off" value={secret} maxLength={512} disabled={busy} onChange={e=>setSecret(e.target.value)}/></label><p className="muted">치지직 개발자 센터에서 앱을 등록하고 채팅 메시지 조회 권한을 선택하세요. 로그인 리디렉션 URL은 아래 주소로 등록해주세요.</p><code className="chzzk-callback">{chat?.chzzkCallback||'http://127.0.0.1:4319/chzzk/callback'}</code><p className="muted">기본 브라우저에서 내 채널의 연결을 승인해요. 비밀 키와 토큰은 저장하지 않으며 만료 후 다시 인증해요.</p></>}
      <button className="secondary" disabled={busy||!ready||!state.running||state.settings.mode!=='live'} onClick={()=>void connect()}>{platform==='youtube'?'YouTube 채팅 연결':'치지직 인증 시작'}</button>{!state.running&&<p className="muted">실제 AI 방송을 시작한 뒤 연결할 수 있어요.</p>}</>}
