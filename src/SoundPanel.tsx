...
export function SoundPanel({sound,enabled,status,level}:{sound?:SoundState;enabled:boolean;status:string;level:number}){
  ...
  return <div className="sound-panel" aria-label="관객이 듣는 소리"><div><b>관객이 듣는 소리</b><span>{status}</span><meter min={0} max={1} value={level} aria-label="시스템 출력 음량"/></div><small>Windows 출력 전체 · 마이크와 별도 · 로컬 분석</small>
