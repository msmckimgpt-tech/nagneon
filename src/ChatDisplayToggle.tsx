...
export function ChatDisplayToggle({shown}:{shown:boolean}){
  ...
  return <div className="chat-display-control"><label title="채팅창과 오버레이에 내 발언 표시"><input type="checkbox" checked={shown} disabled={busy} onChange={()=>void change()}/><span>내 발언 표시</span></label>{error&&<small role="alert">{error}</small>}</div>;
}
