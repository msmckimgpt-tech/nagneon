...
export function NaturalExperiences({state,onError}:{state:State;onError:(message:string)=>void}){
  const [practice,setPractice]=useState(false),[archive,setArchive]=useState<{id:string;title:string;startedAt:number;messageCount:number}[]|null>(null);
  async function loadArchive(){try{const result=await api<{episodes:NonNullable<typeof archive>;seasons:NonNullable<typeof archive>}>('story-archive',undefined,'GET');setArchive([...result.episodes,...result.seasons].sort((a,b)=>b.startedAt-a.startedAt));}catch(error){onError(error instanceof Error?error.message:'기록을 불러오지 못했습니다.');}}
  ...
  <details className="panel feature-body" onToggle={e=>{if(e.currentTarget.open&&archive===null)void loadArchive();}}><summary>예전 기획 방송 기록</summary><p>이전 버전의 기록입니다. 전체 대화와 시즌 기록은 내보내기에 포함됩니다.</p>{archive?.length?archive.map(item=><p key={item.id}><b>{item.title}</b> · {new Date(item.startedAt).toLocaleDateString('ko-KR')} · {item.messageCount}개 대화</p>):<p>{archive?'보관된 기록이 없습니다.':'기록을 불러오는 중…'}</p>}</details>
