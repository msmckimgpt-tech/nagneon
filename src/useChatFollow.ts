...
export function useChatFollow(end:RefObject<HTMLDivElement|null>,revision:string,sessionKey:string,enabled=true){
  ...
  useLayoutEffect(()=>{
    ...
    if(!enabled){if(previous.current.revision!==revision)jump();}
    else if(previous.current.sessionKey!==sessionKey)jump();
