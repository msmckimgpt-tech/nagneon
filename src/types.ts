export type TutorialState = {
  version: 1;
  status: 'new' | 'active' | 'paused' | 'completed' | 'skipped';
  step: string;
  skipped: string[];
  originalMode: 'live' | 'rehearsal' | null;
  originalDisplay: boolean | null;
  arrival: null | {
    id: string;
  ...
  overlayAdjusted: boolean;
  overlayClosed: boolean;
};
import type { GalleryPost } from './CommunityGallery';
import type { ClipSummary } from './HotClips';
import type { EconomyState } from './SpecialStudio';
export type Persona = {
  id: string;
  name: string;
  color: string;
...
export type Game = { id: string; name: string; genre: string; context: string; popularity: number };
export type DiscoverySettings = {
  enabled: boolean;
  arrivalSeconds: number;
  mix: { clip: number; guide: number; fan: number; discussion: number; browse: number };
};
...
export type Message = {
  ...
  transcription?: {
    source: 'microphone';
    correction?: { text: string; confidence: number; reason: string; at: number };
  };
  id: string;
  personaId: string;
  name: string;
...
export type SoundState = {
  ...
  last: null | {
    ...
    silent: boolean;
    volumeDb: number;
    classes: { label: string; score: number; peak: number }[];
    systemSpeech: string;
  };
};
export type State = {
  ...
  externalChat?: {
    ...
    messages: { id: string; platform: string; name: string; text: string; receivedAt: number }[];
  };
  obsInput?: {
    phase: string;
    scenes: string[];
  ...
  onboarding?: {
    version: 1;
    status: 'new' | 'completed' | 'existing' | 'skipped';
    completedAt: number | null;
  };
  connectionProbe?: ProbeState;
  ...
  audience: {
    ...
    members: Record<
      string,
      {
        ...
        seconds: number;
        note?: string;
        aliases: { name: string; at: number }[];
        recognized?: number;
        affinity?: number;
        origin?: { key: string; label: string; firstSeenAt: number };
        joinedAt?: number;
      }
    >;
  ...
  knowledge: {
    name: string;
    seconds: number;
    observations: { id: string; text: string; at: number }[];
...
export type ProbeState = {
  status: 'untested' | 'checking' | 'ready' | 'failed' | 'cancelled';
  startedAt?: number;
  checkedAt?: number;
  latencyMs?: number;
  model?: string;
