...
async function open(t,options={}){const dataDir=options.dataDir||directory();const service=await startServer({port:0,dataDir,localSpeech:false,provider:fake(),...options});t?.after(()=>service.close());clearInterval(service.studio.timer);return {...service,dataDir};}
function req(s,path,body,method=body===undefined?'GET':'POST'){return fetch(s.url+'/api/'+path,{method,headers:{Authorization:'Bearer '+s.accessToken,'X-Backseat-Client':'studio','Content-Type':'application/json'},...(body===undefined?{}:{body:JSON.stringify(body)})});}
function start(service){service.studio.configure({...service.studio.settings,mode:'live',lurkRatio:0});service.studio.start();}
...
test('fresh profile has no hidden waiting audience; settings/API cannot create or edit one',async t=>{
  ...
  assert.equal((await req(service,'settings',{...initial.settings,personas:[...initial.settings.personas,{...initial.settings.personas[0],id:'forged',system:false}]},'PUT')).status,409);
  assert.equal((await req(service,'settings',{...initial.settings,title:'내 방송'},'PUT')).status,200);
  start(service);assert.equal(service.studio.settings.personas.length,1);assert.equal(service.studio.calls,0);
  assert.equal((await req(service,'audience/arrive',{requestId:randomUUID(),name:'내가 고른 이름'})).status,400);
});
...
test('arrival holds, settles once, redacts traits on state/SSE/export and reveals only after purchase',async t=>{
  ...
  const publicState=await(await req(service,'state')).json();assert.match(publicState.settings.personas.find(p=>p.id===id).personality,/PRIVATE_PERSONALITY/);
  assert.equal(publicState.audience.members[id].peers,undefined,'profile does not unlock relationships');
  await req(service,'special/unlock',{kind:'profile',personaId:id,requestId:unlockId});assert.equal(raw(service).economy.balance,70);
  service.studio.economy.change(d=>{d.purchases=d.purchases.filter(p=>p.kind!=='profile');});
  assert.equal(service.studio.world.revealed(id,'profile'),true,'permanent unlock survives receipt history pruning');
  await req(service,'special/unlock',{kind:'profile',personaId:id,requestId:randomUUID()});assert.equal(raw(service).economy.balance,70);
});
...
test('failed final commit refunds the hold without a ghost viewer; profile unlock disk failure takes no points',async t=>{
  ...
  assert.equal(service.studio.economy.data.balance,100);assert.equal(service.studio.world.revealed(personaId,'profile'),false);assert.equal(raw(service).economy.balance,100);
});

for(const mode of ['held','completed'])test(`real process termination ${mode}: restart recovers all-or-nothing settlement and preserves request identity`,async t=>{
  const dataDir=directory(),requestId=randomUUID();const child=spawn(process.execPath,['test/fixtures/arrival-crash.mjs',dataDir,mode,requestId],{cwd:process.cwd(),windowsHide:true,stdio:['ignore','pipe','pipe']});
  let errors='';child.stderr.on('data',b=>errors+=b);const marker=mode==='held'?'HELD':'COMMITTED';
...
test('legacy migration keeps met IDs/history, archives unmet candidates, and does not rerun after restart',async()=>{
  const dataDir=directory();writeFileSync(join(dataDir,'settings.json'),JSON.stringify(defaults));
  const old={members:{momo:{sessions:2,seconds:900,recognized:1,affinity:.4,peers:{},memories:['기존 기억'],note:'개인 메모'}},lore:[],posts:[]};writeFileSync(join(dataDir,'audience.json'),JSON.stringify(old));
  let service=await open(null,{dataDir});try{assert.ok(service.studio.settings.personas.some(p=>p.id==='momo'));assert.ok(!service.studio.settings.personas.some(p=>p.id==='gg'));assert.equal(raw(service).autonomy.retired.gg.reason,'legacy-unmet');assert.equal(service.studio.audience.data.members.momo.note,'개인 메모');}finally{await service.close();}
  const legacy=readFileSync(join(dataDir,'settings.json'),'utf8');writeFileSync(join(dataDir,'settings.json'),'{stale invalid legacy');
  service=await open(null,{dataDir});try{assert.equal(service.studio.settings.personas.length,2);assert.deepEqual(service.studio.audience.data.members.momo.memories,['기존 기억']);assert.equal(readFileSync(join(dataDir,'settings.json'),'utf8'),'{stale invalid legacy');}finally{await service.close();writeFileSync(join(dataDir,'settings.json'),legacy);}
});
...
test('viewer evolution needs witnessed speech and keeps notes, names and prior authors on the same ID',async t=>{
  let args;const service=await open(t,{provider:fake(async value=>{args=value;return value.special?.kind==='audience-arrival'?result({arrival:birth}):result();})});start(service);const {personaId:id}=await meet(service),s=service.studio;let now=Date.now()+700000;s.now=()=>now;
  s.world.change(d=>{d.audience.members[id].seconds=600;});await req(service,`audience/${id}/note`,{text:'SECRET_STREAMER_NOTE'},'PUT');
  const old=s.addMessage(id,'식물 이야기 좋아요');const change={personaId:id,preference:'식물 이야기를 더 즐기게 됨',nickname:'이끼친구',reason:'관심사가 통했다',evidence:'같이 식물 키워보자',sociabilityDelta:.05};
  s.autonomy.evolve([change],change.evidence,[]);assert.equal(s.settings.personas.find(p=>p.id===id).name,birth.name);
  s.autonomy.evolve([change],change.evidence,[id]);assert.equal(s.settings.personas.find(p=>p.id===id).name,'이끼친구');assert.equal(s.audience.data.members[id].note,'SECRET_STREAMER_NOTE');assert.equal(s.audience.data.members[id].aliases[0].name,birth.name);assert.equal(old.personaId,id);assert.equal(old.name,birth.name);
  await s.react({speech:'요즘 식물 키우기에 빠졌어요'});assert.ok(!JSON.stringify(args).includes('SECRET_STREAMER_NOTE'));assert.equal(args.ambient.id,'taste');
  const publicState=s.state();assert.equal(publicState.settings.personas.find(p=>p.id===id).values,undefined);assert.equal(publicState.audience.members[id].note,'SECRET_STREAMER_NOTE');assert.equal(publicState.observation.viewerChanges,undefined);
  const receipt=s.world.data.autonomy.receipts;await req(service,'audience/'+id,undefined,'DELETE');assert.ok(!s.settings.personas.some(p=>p.id===id));assert.equal(s.audience.data.members[id].note,'SECRET_STREAMER_NOTE');assert.deepEqual(s.world.data.autonomy.receipts,receipt);assert.equal(s.messages.find(m=>m.id===old.id).name,birth.name);
});
...
test('ordinary conversation follows the streamer and backs off after a refusal without mode controls',()=>{
  let now=1000000;const ambient=new Ambient({now:()=>now});assert.equal(ambient.context('오늘은 처음으로 보스를 해냈어요').id,'celebration');
  assert.equal(ambient.context('그만하고 쉬고 싶어').quiet,true);assert.equal(ambient.snapshot().active,null);now+=1000;assert.equal(ambient.context('게임 계속 볼게').quiet,true);
  now+=600000;assert.equal(ambient.context('만약에 내가 마법사라면?').id,'improv');now+=300000;assert.equal(ambient.context('화면 봐요'),null);
});

test('failed stop persistence still stops and aborts the broadcast with a visible storage error',async t=>{
  const service=await open(t);start(service);const s=service.studio,save=s.world.save,controller=s.controller;
  s.world.save=()=>{throw Error('stop disk full');};s.autonomy.seconds+=1;
  try{s.stop();assert.equal(s.running,false);assert.equal(controller.signal.aborted,true);assert.equal(s.busy,false);assert.match(s.lastError,/stop disk full/);assert.deepEqual(s.audience.presence,{});}finally{s.world.save=save;}
});

test('streamer notes are excluded from every model payload, including off-stream member maps',async t=>{
  const service=await open(t),p=new OpenAIProvider();
  for(const members of [[{id:'a',note:'DO_NOT_SEND_NOTE',memories:['공개 발언']}],{a:{note:'DO_NOT_SEND_NOTE',memories:['공개 발언']}}]){
    const payload=p.payload({settings:service.studio.settings,history:[],speech:'',offStream:true,audience:{members}});const serialized=JSON.stringify(payload);assert.ok(!serialized.includes('DO_NOT_SEND_NOTE'));assert.ok(serialized.includes('공개 발언'));
  }
});

test('failed opening write never leaves a half-started broadcast',async t=>{
  const service=await open(t),s=service.studio;s.configure({...s.settings,mode:'live'});const save=s.world.save;s.world.save=()=>{throw Error('opening disk full');};
  try{assert.throws(()=>s.start(),/disk full/);assert.equal(s.running,false);assert.equal(s.sessionId,null);assert.equal(s.controller.signal.aborted,true);assert.deepEqual(s.audience.presence,{});}finally{s.world.save=save;}
});
