...
test('Ollama real HTTP request keeps instructions, ordered frames and schema, parses usage',async t=>{
  const requests=[];const server=createServer(async(req,res)=>{let raw='';for await(const chunk of req)raw+=chunk;requests.push({path:req.url,body:JSON.parse(raw),headers:req.headers});res.setHeader('Content-Type','application/json');res.end(JSON.stringify(req.url==='/api/show'?info:{done:true,message:{content:JSON.stringify(observation)},prompt_eval_count:100,eval_count:10}));});await listenBrowserLoopback(server);t.after(()=>{server.close();server.closeAllConnections();});
