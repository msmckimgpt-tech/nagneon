...
test('expired external context invalidates pending responses and clears the display',async()=>{
  ...
  await chat.connect({video:'abcdefghijk',apiKey:'test-api-key'});callback.onBatch({items:[item('1')],liveChatId:'live'});
...
test('external session refuses offline input, retires old callbacks, and invalidates deleted input',async()=>{
  ...
  await assert.rejects(chat.connect({video:'abcdefghijk',apiKey:'test-api-key'}));studio.running=true;
  await chat.connect({video:'abcdefghijk',apiKey:'test-api-key'});callbacks[0].onBatch({liveChatId:'live',items:[item('1')]});
  ...
  await chat.connect({video:'abcdefghijk',apiKey:'test-api-key'});callbacks[0].onBatch({liveChatId:'live',items:[item('late')]});callbacks[0].onState({phase:'failed'});
...
test('authenticated external API feeds live model context without granting advice and stops with the app',async t=>{
  ...
  assert.equal((await post('external/youtube/connect',{video:'abcdefghijk',apiKey:'test-api-key'})).status,200);
