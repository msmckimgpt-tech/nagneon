...
test('the shared live, remembered and clipped gift badge distinguishes virtual points and anonymity',()=>{
  assert.equal(render(undefined),'');assert.match(render({amount:24,anonymous:true}),/가상 후원 24P · 익명/);assert.doesNotMatch(render({amount:24,anonymous:false}),/익명/);
});
