...
test('studio upload is denied; internal resident PNG remains persistent and inaccessible after forgetting',async t=>{
 ...
 assert.equal((await fetch(url,{method:'POST',headers,body:Buffer.from('<svg onload=alert(1) />')})).status,403);
 const response=await fetch(url,{method:'POST',headers,body:png});assert.equal(response.status,403);assert.equal((await response.json()).code,'resident-attachments-only');f.s.social.attach(id,png,'검증.png');const a=f.s.social.detail(id).attachments[0];let media=await req(f,a.url.slice(5));assert.equal(media.status,200);assert.deepEqual(Buffer.from(await media.arrayBuffer()),png);assert.equal(media.headers.get('cache-control'),'no-store');
