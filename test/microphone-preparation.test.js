...
function harness(fetch,getUserMedia){
  ...
  const imports={react,'./useSoundAnalysisSource':{useSoundAnalysisSource:source=>{assert.equal(source,null);return null;}},'./useSystemSound':{useSystemSound:()=>({})},'./useClipBuffer':{useClipBuffer:()=>({})},'./clip-uploads':{},'./api':{api:async()=>({ok:true})},'./speech-flow':{...speechFlow,SpeechQueue:Queue,SpeechOutbox:Outbox},'./microphone-recorder.ts':{startMicrophoneRecorder:()=>{const id=++timerId;timers.add(id);return()=>timers.delete(id);}},'./temporal-frames':{TemporalFrames},'./temporal-capture':{},'./capture-preparation':capturePreparation};
