...
import {mkdir,mkdtemp,writeFile,readFile,cp,symlink,rm} from 'node:fs/promises';
import {join,dirname} from 'node:path';
import {tmpdir} from 'node:os';
import {createPackage} from '@electron/asar';
import {packageSources,verifyPackageSources} from '../scripts/lib/package-sources.mjs';
async function fixture(t){
  const root=await mkdtemp(join(tmpdir(),'nagneon-package-source-')),source=join(root,'source'),stage=join(root,'stage'),folder=join(root,'output');
...
test('complete delivered source comparison includes new modules, frontend assets and all worker scripts',async t=>{
  const f=await fixture(t),result=await verifyPackageSources(f.source,f.folder,f.snapshot);assert.equal(result.passed,true);assert.equal(result.matchingSources,13);
...
test('build-only lockfile and package development settings remain tracked after packaging removes them',async t=>{
  for(const source of ['package-lock.json','package.json']){
    const f=await fixture(t);await writeFile(join(f.source,source),source==='package.json'?JSON.stringify({name:'test',version:'1.0.0',scripts:{start:'changed'}}):'{"lockfileVersion":3}');
...
test('changed current source and a missing manifest cannot pass using only old packaged file hashes',async t=>{
  const f=await fixture(t);await writeFile(join(f.source,'server/temporal-new.js'),'new behavior');let result=await verifyPackageSources(f.source,f.folder,f.snapshot);
...
test('stale frontend chunks and tampered runtime workers fail even when source snapshot is unchanged',async t=>{
  const f=await fixture(t);await writeFile(join(f.stage,'dist/assets/stale.js'),'previous build');await createPackage(f.stage,f.archive);await writeFile(join(f.folder,'resources/speech/speech_worker.py'),'wrong worker');
...
test('a source added after staging is missing from the actual archive',async t=>{
  const f=await fixture(t);await writeFile(join(f.source,'server/new-feature.js'),'new feature');const result=await verifyPackageSources(f.source,f.folder,await packageSources(f.source));
  assert.equal(result.passed,false);assert.ok(result.failures.some(v=>v.includes('missing')&&v.includes('new-feature.js')));
});
test('source directory links and unsupported files are rejected without following private content',async t=>{
  const f=await fixture(t);await mkdir(join(f.root,'private'));await writeFile(join(f.root,'private','secret.js'),'not to package');await symlink(join(f.root,'private'),join(f.source,'server/linked'),process.platform==='win32'?'junction':'dir');
  ...
  const other=await fixture(t);await writeFile(join(other.source,'server/debug.txt'),'not to package');await assert.rejects(packageSources(other.source),/Unexpected/);
});

test('legacy metadata permission does not allow arbitrary or nested server JSON',async t=>{
  for(const name of ['private.json','nested/legacy-season-versions.json']){
    const f=await fixture(t),file=join(f.source,'server',name);await mkdir(dirname(file),{recursive:true});await writeFile(file,'{"private":true}');
