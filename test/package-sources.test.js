...
test('complete delivered source comparison includes new modules, frontend assets and all worker scripts',async()=>{
  const f=await fixture(),result=await verifyPackageSources(f.source,f.folder,f.snapshot);assert.equal(result.passed,true);assert.equal(result.matchingSources,9);
