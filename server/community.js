...
export const GalleryPost=z.object({activityReads:ActivityReads.optional(),id:z.string().min(1).max(100),name:z.string(),text:z.string(),time:z.number().nonnegative(),kind:z.string(),personaId:actor.optional(),title:z.string().max(100).optional(),category:z.enum(categories).optional(),votes:z.array(actor).optional(),comments:z.array(z.object({id:z.string().uuid(),name:z.string().max(100),personaId:actor,text:z.string().max(1000),time:z.number().nonnegative(),parentId:z.string().uuid().nullable(),kind:z.enum(['streamer','ai']),deleted:z.boolean().optional()})).max(150).optional()}).passthrough().superRefine((p,ctx)=>{
