...
export const AudienceData=obj({communityActivity:CommunityActivityData.optional(),members:z.record(actor,obj({sessions:number.int(),seconds:number,recognized:number.int(),affinity:number.max(1),peers:z.record(actor,number),memories:z.array(text),joinedAt:time.optional(),presenceMemory:PresenceMemory.optional(),arrivalClip:ArrivalClipReading.optional()})),lore:z.array(obj({id:z.string().regex(/^[a-zA-Z0-9_-]{1,64}$/).optional(),text,createdAt:time.optional(),expiresAt:time.nullable().optional()})).transform(normalizeLore),posts:z.array(GalleryPost)});
...
export const ClipsData=z.array(obj({activityReads:ActivityReads.optional(),votes:z.array(actor).max(150).refine(v=>new Set(v).size===v.length).optional(),id:z.string().uuid(),title:text,game:text,day:text,participants:z.array(obj({id,name:text})),sessionId:id,createdAt:time,updatedAt:time,scene:text,source:text,comments:z.array(comment),messages:z.array(message),readings:z.array(clipReading).max(150).optional(),video:z.boolean(),audio:z.boolean().optional().default(false),audioEligible:z.boolean().optional().default(false),audioStartedAt:time.optional(),audioEndedAt:time.optional(),thumbnail:z.enum(['png','jpg']).nullable()})).superRefine((clips,ctx)=>{
  ...
  for(const c of clips){
    if(c.audioLayout!==undefined&&!['mixed','separate','microphone-only'].includes(c.audioLayout))ctx.addIssue({code:'custom',message:'클립 소리 구성을 확인하세요.'});
    if(c.voice!==undefined&&typeof c.voice!=='boolean')ctx.addIssue({code:'custom',message:'마이크 클립 표시를 확인하세요.'});
    if(c.voice&&(!(c.video||c.audio)||c.audioLayout!=='separate'||!Number.isFinite(c.voiceStartedAt)||!Number.isFinite(c.voiceEndedAt)||c.voiceEndedAt<=c.voiceStartedAt||c.voiceEndedAt-c.voiceStartedAt>45000||c.voiceStartedAt>c.observedAt||c.voiceEndedAt<c.observedAt))ctx.addIssue({code:'custom',message:'분리된 마이크 클립의 트랙과 시간을 확인하세요.'});
