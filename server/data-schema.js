...
export const AudienceData=obj({communityActivity:CommunityActivityData.optional(),members:z.record(actor,obj({sessions:number.int(),seconds:number,recognized:number.int(),affinity:number.max(1),peers:z.record(actor,number),memories:z.array(text),joinedAt:time.optional(),arrivalClip:ArrivalClipReading.optional()})),lore:z.array(obj({id:z.string().regex(/^[a-zA-Z0-9_-]{1,64}$/).optional(),text,createdAt:time.optional(),expiresAt:time.nullable().optional()})).transform(normalizeLore),posts:z.array(GalleryPost)});
