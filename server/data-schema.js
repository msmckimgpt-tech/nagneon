...
export const AudienceData=obj({members:z.record(actor,obj({sessions:number.int(),seconds:number,recognized:number.int(),affinity:number.max(1),peers:z.record(actor,number),memories:z.array(text),joinedAt:time.optional(),arrivalClip:ArrivalClipReading.optional()})),lore:z.array(obj({text,expiresAt:time})),posts:z.array(GalleryPost)});
