...
const voices = [
  ['존댓말 위주로 차분하게 말한다. 짧은 반응을 선호하고 웃음표현은 드물게 쓴다.', /존댓말|높임말|존대|웃음.*드물/],
  ['반말 위주로 편하게 말한다. 짧은 감탄이나 ㅋㅋ는 웃긴 상황에서만 쓴다.', /반말|말을?\s*놓|ㅋㅋ.*상황/],
  ['처음에는 존댓말을 쓰다가 실제로 친해지면 반말을 조금 섞는다. 관계 변화 전에는 말높임을 급히 바꾸지 않는다.', /친해지.*반말|존댓말.*반말|말높임.*섞/],
  ['말수는 적고 결론부터 짧게 말한다. 관심 없는 때에는 듣는다.', /말수.*적|단답|결론부터|조용|짧게/],
  ['자기 선택을 담백하게 밝힌다. 동의하지 않아도 싸움을 걸지 않는다.', /담백|직설|단호|선택.*밝|의견.*분명/],
  ['상황의 엉뚱한 면을 가끔 받아친다. 친한 사이의 장난도 매번 반복하지 않는다.', /엉뚱|받아치|능청|장난|드립|농담/],
  ...
  ['가벼운 초성·줄임말을 알아듣고 가끔 쓰지만 모든 문장을 커뮤니티 표현으로 채우지는 않는다.', /초성|줄임말|축약|커뮤니티.*표현/]
];
const tensions = [
...
export const individualityInstructions=`special.individuality는 이번 첫 만남에서만 사용하는 내부 창작 방향이다. 이미 만들어 둔 관객이나 실제 인구 통계가 아니다. 유입 동기는 source와 intent를 따르고, interest 안에서 한 가지 구체적인 취향을 새로 정한다. conversation은 관심 있는 순간의 반응 방식과 장기적으로 유지할 대화 어투의 방향이다. preference의 양쪽 중 어디에 더 기우는지와 가끔 반대로 행동할 만한 조건을 하나 정한다. 모든 관객에게 같은 '존중·재촉 안 함'을 개성 대신 쓰지 않는다. 방송 규칙과 명확한 행동 중단은 별도로 항상 지킨다.
personality에는 무엇에 관심을 두고 언제 말하거나 조용해지는지와 함께 대화 어투를 구체적으로 남긴다. 말높임은 존댓말 위주·반말 위주·실제 친밀도에 따라 섞는 방식 중 하나처럼 방향을 정하고, 문장 길이, ㅋㅋ/ㅎㅎ 같은 웃음표현, 초성·줄임말, 감탄, 직설·장난의 정도도 필요한 것만 정한다. 이 기준은 같은 관객이 다음 방송과 다음 모델 호출에서도 이어갈 습관이며 매 장면 새로 뽑는 감정 효과가 아니다. 위 커뮤니티 규범이 있더라도 모두 같은 말투로 복제하지 않고 개인차를 남긴다. values에는 어떤 선택을 왜 선호하는지 담는다. 똑같은 문장을 반복하는 고정 유행어·직무·금지 목록으로 성격을 만들지 않는다. 취미는 관심의 배경이며 모든 대화를 자기 취미 비유로 끌고 가지 않는다. nameStyle을 참고하되 고유한 별명을 스스로 정한다. special.usedNames는 이미 사용 중인 공개 닉네임이다. 같은 이름이나 숫자·기호만 덧붙인 변형은 피하고 별개의 이름을 만든다. 다른 관객의 성격·친분은 이 목록으로 알 수 없다. sociability와 expertise는 제시된 수치로 두고 그 정도에 맞게 구성한다. expertise는 실제 특정 게임을 알거나 방송을 봤다는 증거가 아니다. 이 방향의 제목이나 제작 과정을 관객의 말로 읽지 않는다. 기존 사람·스트리머와의 친분·구체적 과거 방송 경험을 만들지 않는다.`;
