...
import { viewerSpeechStyle } from './viewer-speech-style.js';
import { playfulPushbackAllowed } from './viewer-speech-adaptation.js';

const clamp = (v) => Math.min(1, Math.max(0, v));
const round = (v) => Math.round(v * 100) / 100;

// 게임 인지도와 페르소나 숙련도로 만드는 일반적 배경 지식(개인 관찰 이력과 무관).
export function generalFamiliarity(popularity = 0.5, expertise = 0.5) {
  return clamp(popularity * 0.4 + expertise * 0.4);
}
// 본인이 이 방송에서 직접 시청한 시간으로만 쌓이는 개인적 숙지도.
export function personalFamiliarity(watchedSeconds = 0) {
  return clamp(Math.log2(1 + Math.max(0, watchedSeconds) / 3600) * 0.3);
}
...
export function viewerKnowledge(
  ...
  const witnessed = observations
    .filter((o) => Array.isArray(o.witnesses) && o.witnesses.includes(viewerId))
    .slice(-witnessLimit)
    .map((o) => ({ text: o.text, at: o.at }));
  // provenance 가 없는(레거시) 관찰만 공용 배경으로. 다른 관객이 목격해 witnesses 가 채워진 장면은 여기에서 제외되어 개인 기억이 새지 않는다.
  const priorScenes = observations
    .filter((o) => !Array.isArray(o.witnesses) || o.witnesses.length === 0)
    .slice(-sharedLimit)
    .map((o) => o.text);
  ...
  return {
    ...
    taughtNotes: Array.isArray(entry?.notes)
      ? entry.notes.slice(-noteLimit).map((n) => n.text)
      : [], // 스트리머가 알려준 공용 지식
...
export function viewerKnowledgeByPersona(entry, personas = [], { popularity = 0.5 } = {}) {
  ...
  for (const p of personas)
    map[p.id] = viewerKnowledge(entry, {
      viewerId: p.id,
      expertise: p.expertise ?? 0.5,
      popularity,
    });
  return map;
}
...
export function liveViewerContext(
  audience,
  personas,
  history,
  previous,
  {
    ...
    externalChat,
    privateMembers = {},
    addressViewers = createViewerAddressResolver(personas, audience.members, now),
  } = {},
) {
  const packets = {},
  ...
  const microphoneWitnesses = journal
    ? new Map(
        journal.data.entries
          .filter((e) => e.transcription?.source === 'microphone')
          .map((e) => [e.id, new Set(e.witnesses)]),
      )
    : null;
  for (const p of personas) {
    const member = audience.members.find((m) => m.id === p.id);
    const privateMember = privateMembers[p.id] || member;
    const joinedAt = member?.joinedAt;
    const witnessed = Number.isFinite(joinedAt)
      ? history.filter(
          (m) =>
            m.time >= joinedAt &&
            m.time <= now &&
            (m.transcription?.source !== 'microphone' ||
              (!transcriptAnomaly(m.text) &&
                (!microphoneWitnesses || microphoneWitnesses.get(m.id)?.has(p.id)))),
        )
      : [];
    packets[p.id] = {
      ...(social ? { heardFromCommunity: social.memory(p.id) } : {}),
      joinedAt,
      speechStyle: viewerSpeechStyle(p, privateMember),
      ...(privateMember?.speechStyleBoundary
        ? {
            speechStyleBoundary: {
              axis: privateMember.speechStyleBoundary.axis,
              target: privateMember.speechStyleBoundary.target,
            },
          }
        : {}),
      ...(privateMember?.speechStyleAdaptation &&
      privateMember.speechStyleAdaptation.stage !== 'adopted'
        ? {
            speechStyleLearning: {
              stage: privateMember.speechStyleAdaptation.stage,
              axis: privateMember.speechStyleAdaptation.axis,
              target: privateMember.speechStyleAdaptation.target,
            },
          }
        : {}),
      playfulPushback: {
        allowed: playfulPushbackAllowed(witnessed, p.id, addressViewers, now, privateMember),
      },
      preferences: structuredClone(member?.preferences || []),
      heardSounds: sound?.context(p.id) || [],
      memories: journal ? [] : structuredClone(member?.memories || []),
      ...
      ...(clips
        ? {
            clipMemories: clips.recall(p.id, speech, now),
            arrivalClipMemory: clips.recallArrival(member?.arrivalClip, now),
          }
        : {}),
      ...
      conversationRhythm: conversationRhythm(witnessed, p.id, {
        now,
        speech,
        previousScene: previous?.at >= joinedAt ? previous.scene : '',
        name: p.name,
        addressed: addressed.has(p.id),
        addressViewers,
      }),
      watchTiming: {
        receivedAt: now,
        ...(viewing?.timing[p.id] || {}),
        previousAnalysisAgeSeconds:
          Number.isFinite(joinedAt) && previous?.at >= joinedAt && previous.at <= now
            ? Math.floor((now - previous.at) / 1000)
            : null,
      },
      previous:
        Number.isFinite(joinedAt) && previous?.at >= joinedAt ? structuredClone(previous) : null,
    };
  }
  const members = audience.members.map(
    ({
      memories,
      note,
      preferences,
