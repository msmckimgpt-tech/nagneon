...
export class LocalSpeech {
  constructor(runtime={},spawner=spawn){this.runtime=runtime;this.spawn=spawner;this.child=null;this.ready=false;this.pending=null;this.error='';this.model='';}
  start(){
    ...
    this.child=this.spawn(python,args,{windowsHide:true,env:{...process.env,PYTHONIOENCODING:'utf-8',...(model?{HF_HUB_OFFLINE:'1'}:{})}});let buffer='';
    this.child.stdout.setEncoding('utf8');
    this.child.stdout.on('data',b=>{buffer+=b;let n;while((n=buffer.indexOf('\n'))>=0){const line=buffer.slice(0,n);buffer=buffer.slice(n+1);try{const data=JSON.parse(line);if(data.ready){this.ready=true;this.model=data.model||'';}else if(data.error&&!data.id){this.ready=false;this.error=data.error;const p=this.pending;this.pending=null;p?.reject(new Error(data.error));}else if(this.pending&&data.id===this.pending.id){const p=this.pending;this.pending=null;data.error?p.reject(new Error(data.error)):p.resolve({text:data.text,cues:data.cues,...(data.timing?{timing:data.timing}:{})});}}catch{}}});
