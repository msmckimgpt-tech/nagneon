...
export class ConnectionProbe{
  ...
  async run(provider=this.provider){
    ...
    try{
      const result=await provider.react({connectionProbe:true,settings,history:[],speech:'연결 확인입니다. 짧은 한국어 인사 한 문장으로 응답해주세요.',previous:null,knowledge:null,audience:{eligible:['probe'],members:[{id:'probe',presence:'active'}],lore:[],offStreamPosts:[]},adviceRequested:false},controller.signal);
