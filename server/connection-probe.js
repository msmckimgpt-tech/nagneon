...
export class ConnectionProbe{
  ...
  async run(provider=this.provider){
    if(this.controller)throw new Error('이미 응답을 확인하고 있습니다.');
    if(!provider.status().configured)throw new Error('선택한 모델 연결을 먼저 확인해주세요.');
    ...
    try{
      const result=await provider.react({aiFeature:'probe',connectionProbe:true,settings,history:[],speech:'연결 확인입니다. 짧은 한국어 인사 한 문장으로 응답해주세요.',previous:null,knowledge:null,audience:{eligible:['probe'],members:[{id:'probe',presence:'active'}],lore:[],offStreamPosts:[]},adviceRequested:false},controller.signal);
      ...
      provider.aiControl?.accepted(result);
      this.value={status:'ready',checkedAt:Date.now(),latencyMs:Date.now()-started,model:result.routing?.model||provider.model,effort:result.routing?.effort||provider.effort,reply:reply.slice(0,240),tokens:Number(result.usage?.total_tokens)||0};
