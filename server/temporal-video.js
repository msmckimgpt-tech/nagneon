...
export const temporalInstructions=`screenTimeline이 있으면 첨부 이미지들은 같은 방송에서 시간순으로 추린 화면 표본이다. 사진첩·화보·쇼츠를 넘길 때는 서로 다른 사진이나 영상일 수 있으므로 같은 인물·몸·동작으로 합쳐 해석하지 않는다. frames.index가 첨부 순서이며 capturedAt, gapMs, ageMs는 실제 수집 시각과 간격이다. 첫 장부터 마지막 장까지 대상의 이동, 시작-시도-결과, 화면 전환을 연결해서 이해하고 마지막 장을 현재 상태로 삼는다. scene에는 확실히 보인 변화와 마지막 상태를 간결히 담는다. 화면 전환은 시점 파악을 위한 단서이지 채팅의 기본 주제가 아니다. 사진·영상 감상 채팅은 마지막 화면의 실제 콘텐츠에 반응하고, 이전 사진과 비교할 때만 이전 표본을 구분해서 참고한다. 한 장뿐이면 움직임을 단정하지 않는다. sampled=true는 중간 화면이 생략됐다는 뜻이다. 큰 시간 간격이나 화면 전환 너머의 과정, 버튼 입력, 실패 원인, 소리를 만들어 잇지 않는다. sourceChanged 또는 gapBeforeMs가 크면 이전 장면에서 계속 이어졌다고 단정하지 않는다.
...
still은 동일한 픽셀인 중간 표본을 줄인 기록이다. since부터 해당 capturedAt까지 samples개 표본이 같았으므로 첨부 이미지가 적다는 이유로 중간을 전혀 못 봤다고 말하지 않는다. 표본 사이의 모든 순간까지 정지였다고 단정할 필요는 없다. 평소에는 '그대로 있네'처럼 상태에 답하며 자료 개수나 전송 방식을 설명하지 않는다. 사용자가 인식 방식이나 놓친 부분 자체를 물을 때만 정확한 한계를 설명한다.
