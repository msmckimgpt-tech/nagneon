...
export function adviceIntent(speech=''){
  ...
  for(const clause of text.split(/(?<=[.!?。！？\n])|(?<=말고)\s+|\s*(?:그런데|하지만|근데|대신)\s*/u)){
    ...
    const choice=/(?:골라|고르|선택|추천)/u.test(clause);
    ...
    if(/(?:부탁|요청|알려\s*달라|도와\s*달라).{0,12}(?:했|했던|했었|하던|했더|했잖|고\s*말했)/u.test(clause)
      ||(choice&&/(?:달라|줘|주세요|줄래).{0,8}(?:라고|고)\s*(?:했|말했|하던|했던|했었|했더|했잖)/u.test(clause)))continue;
    if((topic&&/(?:부탁|(?:해|알려|가르쳐|찾아|검색해|설명해)\s*(?:줘|주|줄)|(?:하나|조금|좀|만)\s*(?:줘|주|줄)|듣고\s*싶|가능할까|해도\s*돼)/u.test(clause))
      ...
      ||/(?:골라|추천해|선택해)\s*(?:줘요?|주세요|주실래요?|줄래요?|줄\s*수\s*있(?:어|을까|나요|어요))(?=$|[\s,.!?。！？~ㅋㅎ])/u.test(clause)
      ||(/(?:뭐|뭘|무엇을|어느\s*(?:거|것)|어떤\s*(?:거|것|카드))\s*(?:를\s*)?(?:골라\s*볼까|고를까|고르면\s*(?:좋|될)|선택할까)/u.test(clause)
        &&!/(?:골라\s*볼까|고를까|선택할까)(?:라고|고)?\s*(?:했|생각|혼잣말)/u.test(clause))
      ||/(?:어떻게|어디로|어디를|뭘|무엇을|어느\s*쪽|어떤\s*카드).{0,25}(?:해야|하면|풀|깨|이기|가야|골라|고르|선택|써야|눌러|누르면)/u.test(clause)){
      requested=true;refused=false;
      single=/(?:하나\s*만|한\s*(?:가지|개|장)\s*만|딱\s*(?:하나|한\s*(?:가지|개|장))|\b(?:one|single)\s+hint\b)/u.test(clause);
    }
  }
