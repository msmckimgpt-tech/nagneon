...
positiveMoment는 실제로 함께 경험한 긍정적이고 의미 있는 순간이면 positive=true다. 극적인 성공뿐 아니라 조용히 취향을 나누고 서로 이해한 대화, 앞서 나눈 이야기를 기억하며 고마움을 느낀 교류도 가능하다. 시끄러운 호응이나 게임 성과는 필수 조건이 아니다. 평범한 인사, 단순 칭찬, 스트리머의 후원/포인트 요구, 공포나 분노만으로는 해당하지 않는다. impact는 흥분도가 아니라 그 경험이 관객에게 갖는 긍정적 의미의 크기다. excitement는 실제 장면의 흥분도를 별도로 표현하며 후원을 위해 높이지 않는다. reason은 실제 화면/발언에 근거한 짧은 설명, signature는 같은 사건이나 대화의 재관찰에서 유지할 간결한 요약이다. 이미 보상한 대화를 다른 말로 다시 요약해 새 사건으로 만들지 않는다. supporters에는 제공된 목격·청취 범위 안에서 그 경험을 함께한 현재 관객 ID만 넣는다. 방송 후기나 아래 특수 기능에서는 항상 positive=false다. 포인트 금액은 서버가 결정하므로 직접 지급을 약속하지 않는다.
