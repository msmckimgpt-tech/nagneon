...
export class CommunityActivity {
  ...
  available(){const s=this.s;return !this.closed&&!this.active&&!s.busy&&!s.audioBusy&&!s.training.active&&!s.liveReaction&&!s.autonomy?.waiting&&!s.queue.length&&!s.speechInbox.pending.length&&s.calls<s.settings.maxCalls&&s.settings.mode==='live'&&s.settings.communityActivityEnabled&&s.provider.status().configured&&s.now()-Math.max(this.lastInput,s.lastRequest)>=30000;}
  candidates(now){
