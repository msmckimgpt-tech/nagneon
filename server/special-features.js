...
export class SpecialFeatures {
  ...
  person(id){const s=this.ready(),p=s.settings.personas.find(p=>p.id===id);if(!p||!s.audience.data.members[id]?.sessions)throw new Error('아직 함께 방송을 본 관객이 아닙니다.');return p;}
