...
export const DebugConfig=z.object({enabled:z.boolean(),tryNewFeatures:z.boolean().default(false),mode:z.enum(['append','replace']),prompt:z.string().max(60000)}).strict();
export const initialDebug=()=>({enabled:false,tryNewFeatures:false,mode:'append',prompt:''});
...
export function debugRoutes(app,studio,store,{idle=()=>true,onChange=()=>{}}={}){
  ...
  app.get('/api/debug',(_req,res)=>{
    if(!previewEnabled(config))return res.json({config:read()});
  ...
  app.put('/api/debug',(req,res)=>{requireIdle();const next=DebugConfig.parse(req.body);store.save(next);config=next;onChange(next);studio.publish();res.json({config:read()});});
  app.put('/api/debug/settings',(req,res)=>{
    requireIdle();if(!previewEnabled(config))throw Error('디버그 모드와 신규 기능 사용해보기를 모두 켜주세요.');
  ...
  return {read,summary:()=>({enabled:config.enabled,tryNewFeatures:config.tryNewFeatures,previewEnabled:previewEnabled(config),customPrompt:previewEnabled(config)&&(config.mode==='replace'||!!config.prompt.trim())})};
}
