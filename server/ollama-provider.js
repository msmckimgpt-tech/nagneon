...
export class OllamaProvider extends OpenAIProvider {
  constructor(env=process.env,fetcher=fetch){
    ...
    this.base=url.origin;this.model=env.OLLAMA_MODEL||'';this.effort='local';this.transcriptionModel='로컬 음성 인식';this.available=false;this.vision=false;this.message='Ollama와 사용할 로컬 모델을 설정해주세요.';this.think=env.OLLAMA_THINK===undefined?undefined:env.OLLAMA_THINK==='true';
  ...
  async localRequest(path,body,signal,timeout=60000){
    ...
    }catch{if(combined.aborted)throw Object.assign(Error('Ollama 요청이 취소되었거나 응답 시간이 지났습니다.'),{code:'timeout'});throw Object.assign(Error('Ollama 응답을 받지 못했습니다. 로컬 서버와 모델 설정을 확인해주세요.'),{code:'network'});}
  }
  async check(signal){
  ...
  async react(args,signal){
    ...
    if(!this.available)throw Object.assign(Error(this.message),{code:'unavailable'});
