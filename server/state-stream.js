...
export class StateFeed {
  constructor(response,{patches=false,currentState,enabled=()=>true}){
    this.enabled=enabled;this.patches=patches;
    this.response=response;this.currentState=currentState;this.encoder=null;this.blocked=false;this.dirty=false;this.closed=false;
  ...
  send(state){if(this.closed)return;if(this.blocked){this.dirty=true;return;}if(this.patches&&this.enabled())this.encoder??=new StateStream();else this.encoder=null;this.write(this.encoder?this.encoder.encode(state):`data: ${JSON.stringify(state)}\n\n`);}
