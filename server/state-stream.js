...
export class StateStream {
  ...
  encode(state) {
    ...
    // Compare serialized values, never object identity: callers mutate nested state.
    if (serialized === this.serialized) return '';
    const current = JSON.parse(serialized),
      fields = new Map(Object.entries(current).map(([key, value]) => [key, JSON.stringify(value)]));
    if (!this.previous || Object.keys(this.previous).some((key) => !(key in current))) {
    ...
    for (const [key, value] of Object.entries(current)) {
      if (fields.get(key) === this.fields.get(key)) continue;
      if (key !== 'messages') {
        set[key] = value;
        continue;
      }
      const before = this.previous.messages || [],
        after = value;
      const old = new Map(before.map((m) => [m.id, m])),
        next = new Set(after.map((m) => m.id));
      ...
      const expected = [
        ...before.filter((m) => next.has(m.id)).map((m) => m.id),
        ...after.filter((m) => !old.has(m.id)).map((m) => m.id),
      ];
      ...
      if (JSON.stringify(delta).length < JSON.stringify(after).length) messages = delta;
      else set.messages = after;
    }
    this.previous = current;
    this.serialized = serialized;
    this.fields = fields;
    ...
    return patch.length < serialized.length
      ? `event: state-patch\ndata: ${patch}\n\n`
      : `data: ${serialized}\n\n`;
  }
}
