...
const privacyFields={acknowledgeAiTransfer:z.literal(true),shareNames:z.boolean().default(false)};
...
export class ExternalChatSession {
  ...
  async beginChzzk({acknowledgeAiTransfer,shareNames=false,...credentials}){
    z.object(privacyFields).parse({acknowledgeAiTransfer,shareNames});
    ...
    const auth=this.authFactory({onState:status=>{if(!current()||status.phase==='authorized')return;if(status.phase==='failed'){this.authorizationUrl='';this.buffer.clear();}this.status=status;s.publish();},onAuthorized:async(tokens,signal)=>{
      ...
      const transport=this.chzzkFactory({onState:status=>{if(!current())return;this.status=status;
        if(status.phase==='receiving'&&!sourceId){this.authorizationUrl='';sourceId=this.buffer.open({platform:'chzzk',channelId:status.channelId,sessionId,shareNames});}
  ...
  async connect({video,apiKey,acknowledgeAiTransfer,shareNames=false}){
    z.object(privacyFields).parse({acknowledgeAiTransfer,shareNames});
    const s=this.studio;if(!s.running||s.settings.mode!=='live')throw Error('실제 AI 방송을 시작한 뒤 외부 채팅을 연결해주세요.');
    const channelId=youtubeVideoId(video);this.disconnect();const generation=this.generation,sessionId=s.sessionId;
    const sourceId=this.buffer.open({platform:'youtube',channelId,sessionId,shareNames});
...
export function externalChatRoutes(app,studio,options){
  ...
  app.post('/api/external/youtube/connect',async(req,res)=>{
    const input=z.object({video:z.string().min(1).max(2048),apiKey:z.string().min(10).max(256),...privacyFields}).strict().parse(req.body);if(res.destroyed)return;
  ...
  app.post('/api/external/chzzk/start',async(req,res)=>{
    const input=z.object({clientId:z.string().min(8).max(512),clientSecret:z.string().min(8).max(512),...privacyFields}).strict().parse(req.body);if(res.destroyed)return;
