...
const failureMessages={usage:'ChatGPT 사용량 한도에 도달했습니다. 한도 회복 후 다시 시도하거나 리허설로 이어가세요.',model:'선택한 관객 모델을 이 계정에서 사용할 수 없습니다. 모델 설정과 계정의 접근 권한을 확인해주세요.',auth:'ChatGPT 인증을 확인하지 못했습니다. 방송 설정에서 계정을 다시 연결해주세요.',network:'모델 서버 연결이 끊겼습니다. 인터넷 연결을 확인한 뒤 다시 시도하세요.',unknown:'관객 응답을 완료하지 못했습니다. 계정 연결과 모델 접근 상태를 확인해주세요.'};
  ...
  async react(args,signal){
    ...
    try{
      ...
      const usage=await new Promise((resolve,reject)=>{
        const child=this.spawn(this.bin,command,{windowsHide:true,cwd:dir,stdio:['pipe','pipe','pipe'],env:this.env});let buffer='',usage={},done=false,terminationError=null,killDeadline,failureKind='unknown';
        ...
        child.stdout.on('data',chunk=>{buffer+=chunk;let n;while((n=buffer.indexOf('\n'))>=0){const line=buffer.slice(0,n);buffer=buffer.slice(n+1);try{const event=JSON.parse(line);if(event.type==='turn.completed'){const u=event.usage||{};usage={input_tokens:u.input_tokens,cached_input_tokens:u.cached_input_tokens,output_tokens:u.output_tokens,total_tokens:Number.isFinite(u.input_tokens)&&Number.isFinite(u.output_tokens)?u.input_tokens+u.output_tokens:undefined};args.onAiUsage?.(Object.keys(u).length?usage:null);}else if(['error','turn.failed'].includes(event.type)){const kind=codexFailure(String(event.message||event.error?.message||'').slice(0,8192));if(kind!=='unknown')failureKind=kind;}}catch{}}if(buffer.length>1_000_000)buffer='';});
