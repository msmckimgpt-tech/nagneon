...
export class SpeechInbox {
  ...
  receive(id,text,publish,source='keyboard'){
    const fingerprint=createHash('sha256').update(source+'\0'+text).digest('hex'),prior=this.receipts.get(id);
    ...
    this.receipts.set(id,{fingerprint,messageId:message.id});this.pending.push({id,text,messageId:message.id,source});
